...
class BellWongTest < Test::Unit::TestCase
  ...
  context "with the 'myopic' test network" do
    ...
    [BWNNHandler, BWH1Handler, BWH2Handler].each do |reactive_handler|
      should "move the wrong vehicle with #{reactive_handler} (alpha 0.4)" do
        if reactive_handler == BWNNHandler
          @sim.reactive = BWNNHandler.new(@sim)
        elsif reactive_handler == BWH1Handler
          @sim.reactive = BWH1Handler.new(@sim, @od, 0.4)
        else
          @sim.reactive = BWH2Handler.new(@sim, @od, 0.4, 1)
        end
        @sim.init
    ...
    [BWH1Handler, BWH2Handler].each do |reactive_handler|
      should "move right vehicle with #{reactive_handler} and alpha > 0.5" do
        if reactive_handler == BWH1Handler
          @sim.reactive = BWH1Handler.new(@sim, @od, 0.6)
        else
          # for pax 0 -> 2 @ 0: w(0) = 10 and w(1) = 30
          # to get value(0) for horizon 1, note that the expected wait for the
          # next passenger if only vehicle 1 is available is just t_{21} = 40,
          # because @od has only d_{1 \sigma} > 0. This gives
          #   value(0) = w(0) + alpha*t_{21} = 10 + alpha*40
          # so 0.5 is again the critical value, in this case
          @sim.reactive = BWH2Handler.new(@sim, @od, 0.6, 1)
        end
        @sim.init
  ...
  context "BWH1/H2 test on three station star network" do
    #
    ...
    [BWNNHandler, BWH1Handler, BWH2Handler].each do |reactive_handler|
      should "move the vehicle at 1 with #{reactive_handler} (alpha 0.0)" do
        if reactive_handler == BWNNHandler
          ...
          @sim.reactive = BWH2Handler.new(@sim, @od, 0.0, 2)
        end
        @sim.init
    ...
    [BWH1Handler, BWH2Handler].each do |reactive_handler|
      should "not move vehicle at 1 with #{reactive_handler} if alpha big" do
        if reactive_handler == BWH1Handler
          @sim.reactive = BWH1Handler.new(@sim, @od, 0.75)
        else
          @sim.reactive = BWH2Handler.new(@sim, @od, 0.75, 1)
        end
        @sim.init
